...
class cudaBayesian(Bayesian, family="altar.models.cudabayesian"):
    ...
    # set during initialization from the parameter sets
    parameters = None
    ...
    psets_list.doc = "list of parameter set names that defines the order (for embedded views)"

    parametersets = altar.cuda.models.parameters()
    parametersets.doc = "a parameter set or an ensemble of parameter sets in the model"
    ...
    case = altar.properties.path(default=None)
    ...
    def initialize(self, application):
        ...
        if self.embedded is False: # base model
            ...
            if hasattr(parametersets, "psets"):
                self.psets = parametersets.psets
                self.psets_list = parametersets.psets_list
            else:
                self.psets = {"parameters": parametersets}
                self.psets_list = ["parameters"]
        else: # an embedded model in an ensemble
            # share parameter sets from the master model
            self.parametersets = application.model.parametersets
            parametersets = self.parametersets

            if hasattr(parametersets, "psets"):
                if not self.psets_list:
                    channel = self.error
                    channel.log("embedded model requires 'psets_list' to define a view of parametersets")
                    raise SystemExit(1)
                # build a view into the global parameter sets
                try:
                    self.psets = {name: parametersets.psets[name] for name in self.psets_list}
                except KeyError as error:
                    channel = self.error
                    channel.log(f"unknown parameter set name '{error.args[0]}' in psets_list")
                    raise SystemExit(1)
            else:
                # single parameter set shared by the parent
                self.psets = {"parameters": parametersets}
                if self.psets_list is None:
                    self.psets_list = ["parameters"]

            # compute the parameter count from this view
    ...
    def _iter_psets(self):
        ...
        """
        psets = getattr(self, "psets", None)
        if psets is None:
            return
        psets_list = getattr(self, "psets_list", None)
        if psets_list:
            for name in psets_list:
                yield psets[name]
        else:
            yield from psets.values()
