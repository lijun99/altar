...
const char * const altar::extensions::distribution::verify__name__ = "distribution_verify";
const char * const altar::extensions::distribution::verify__doc__ =
    "verify samples for a ranged distribution";

PyObject *
altar::extensions::distribution::verify(PyObject *, PyObject * args) {
    // the arguments
    double low, high;
    ...
    int status = PyArg_ParseTuple(
                                  args, "(dd)O!O!:distribution_verify",
                                  &low, &high,
    ...
    if (!PyCapsule_IsValid(maskCapsule, altar::vector::capsule_t)) {
        PyErr_SetString(PyExc_TypeError, "invalid mask/vector capsule");
        return 0;
    }
    // bail out if the {theta} capsule is not valid
    if (!PyCapsule_IsValid(thetaCapsule, gsl::matrix::view_t)) {
        PyErr_SetString(PyExc_TypeError, "invalid theta/matrix capsule");
        return 0;
    }
