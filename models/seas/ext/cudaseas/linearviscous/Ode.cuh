...
struct __ALIGNED__ Ode {
    ...
    T *alpha1, // viscous coefficient [systems, parameters]
    T Vj, // backslip rate
    ...
    __device__ __forceinline__ void dydt(const int system_id, const int patch_id, const T t, const T* y, T* f)
    {
        ...
        // dvdt
        // compute dtau/dt temporarily
        auto ix = patches+patch_id
        f[ix] = stressrate_ext[ix];
    ...
    // pass the pre-allocated parameter pointers
    void init_parameters(
        const T Vj_, const T* stress_kernel_, T* stressrate_ext_)
    {
        Vj = Vj_;
