...
struct __ALIGNED__ Controller
{
    ...
    __device__ auto success(const cg::thread_block & cta, Stepper<T>& s, T&h)->bool;
    ...
    __device__ void set_init_h(const cg::thread_block & cta, const T h)
    {
        if(cta.thread_rank()==0)
        {
            hnext = static_cast<T>(0.01)*h;
        }
        cta.sync();
    };
};
    ...
    Stepper<T>& s, T& h ) -> bool
{
	...
	if(cta.thread_rank() == 0)
	{
        ...
        if (err <= static_cast<T>(1.0))
        {
            ...
            if (reject) // check whether h was rejected in the
                hnext=h*min(scale,static_cast<T>(1.0));
            else
                hnext=h*scale;
...
__device__ T Controller<T>::find_initial_step(
    ...
    Stepper<T>& s, OdeFuncType& dydt_patch, Args... args)
 {
    ...
    d0 = cuda::detail::sum_block<T, decltype(d0_norm_func)>(N, d0_norm_func);
    ...
    d1 = cuda::detail::sum_block<T, decltype(d1_norm_func)>(N, d1_norm_func);
    ...
    cuda::detail::block_process<T, decltype(y1func)>(N, y1func);
    cta.sync();
    ...
    d2 = cuda::detail::sum_block<T, decltype(d1_norm_func)>(N, d1_norm_func);
