...
struct __ALIGNED__ SEASEvents {
    ...
    int system_size;
    int num_eq;
    T * ychange; // [num_eq=nevents-2, patches * 2] - first all velocities in one direction, then the other
    ...
    SEASEvents (const int nevents_, const T* tevents_, const T* delta_tau, const T* alpha_h_vec, const int patches_, const int units_)
        : nevents(nevents_), patches(patches_), units(units_)
    {
        ...
        cudaMallocManaged(&ychange, num_eq * patches * 2 * sizeof(T));
        for (auto ievent = 0; ievent < num_eq; ievent++)
            for (auto i = 0; i < patches; i++) {
                ychange[ievent * patches * 2 + i] = (T) delta_tau[i_dtau(ievent, i, 0)] / alpha_h_vec[i];
                ychange[ievent * patches * 2 + patches + i] = (T) delta_tau[i_dtau(ievent, i, 1)] / alpha_h_vec[i];
            }
    }
    ...
    __device__ void set_events_block(const cg::thread_block & cta,
        const int system_id, const int event_id, T* yn) // TODO does this function have to deal with all systems?
    {
        if (event_id == 0) return;

        auto yevent = ychange + (event_id - 1) * patches * 2;
